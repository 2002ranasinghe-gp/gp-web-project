...
if(isset($_POST['patsub1'])){
    ...
    $password = $_POST['password']; // Don't escape password before storing
    ...
    $check_nid = "SELECT * FROM patreg WHERE national_id='$national_id'";
    $nid_result = mysqli_query($con, $check_nid);
    
    if(mysqli_num_rows($nid_result) > 0){
        echo "<script>alert('National ID already exists! Please check your National ID.'); window.location.href = 'index.php';</script>";
        exit();
    }
    
    // INSERT PATIENT WITHOUT CPASSWORD - FIXED LINE 43
    $query = "INSERT INTO patreg (fname, lname, gender, email, contact, national_id, password) 
              VALUES ('$fname','$lname','$gender','$email','$contact','$national_id','$password')";
    ...
    if($result){
        ...
        echo "<script>
            alert('Registration Successful! Welcome to Healthcare Hospital.');
            window.location.href = 'patient_dashboard.php';
        </script>";
        exit();
    } else {
        echo "<script>
            alert('Registration Failed: " . addslashes(mysqli_error($con)) . "');
...
<html lang="en">
  ...
  <body style="padding-top:50px;">
      ...
      <div class="jumbotron" id="ab1" style="background: url(\'images/hospital-bg.jpg\') no-repeat center center; background-size: cover; height: 300px;"></div>
              ...
              <div class="tab-pane fade" id="list-messages" role="tabpanel" aria-labelledby="list-messages-list">
                <div class="card">
                  <div class="card-body">
                    ...
                    <p>Prescription functionality will be implemented here.</p>
                  </div>
                </div>
              </div>
                ...
                <form class="form-group" method="post" action="func.php">
                  <div class="card">
                    <div class="card-body">
                      ...
                      <input type="text" name="name" placeholder="enter doctor\'s name" class="form-control" required>
                      <br>
